...
源码抽取曾因所有 HTML 一律上报而意外扩展为产品文档任务。本协议现在仅覆盖明确的产品文档工作流；运行时范围定义统一位于 `src/html-delivery.mjs` 的 `HTML_DELIVERY_SCOPE`，由会话 Hook 和 MCP 共用。纯页面抽取、隔离预览与未纳入该流程的功能试做直接交付，不启动文档同步。以下原有同步步骤仅在此范围内适用。

保持工具参数、路径校验、批次标识和正式产品文档的同轮同步要求不变，不增加页面模式或自动目录扫描。否决仅在抽取 Skill 添加豁免的做法，因为全局 Hook 仍会覆盖它。回归检查会话注入、MCP 工具契约和 HTML 路径校验；插件源码需随后正常发布安装才影响新会话。
