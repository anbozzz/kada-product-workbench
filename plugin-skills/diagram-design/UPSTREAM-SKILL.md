...
When the current task is inside `workplace-ai/`, read and follow [`references/workplace-integration.md`](references/workplace-integration.md) before the type-specific workflow. That local contract preserves Project routing, source coverage, existing PlantUML/Mermaid maintenance sources, output locations, and workspace validation. It overrides only conflicting file-organization and lifecycle defaults; the upstream Diagram Design type, layout, accessibility, safety, and taste gates remain authoritative.

- Keep formal outputs under the selected Project's `drafts-diagrams/{topic}/` and preserve `coverage.md`.
- Existing PlantUML/Mermaid maintenance sources remain canonical until the user explicitly approves migration.
- New visual deliverables default to self-contained Diagram Design HTML; SVG/PNG, PlantUML, Mermaid, and FigJam remain request-driven outputs.
- Validate coverage, run the packaged `self_check.py`, inspect changed HTML in a browser, and synchronize the workspace atlas only when global routing or ownership changes.
