...
> 发布前草稿 · 2026-09-21  
> 按当前 0.12.6 候选版本整理，供修改后用于 GitHub 项目介绍。安装包已在本地生成并完成 macOS 隔离安装验证，尚未公开上传 GitHub。本文没有虚构下载地址，也不代表所有操作系统均已验收。
...
![PRD 持续评审：选中原文、汇总意见、由原 Codex 任务修订，再查看新版](assets/kada-feature-prd.png)

直接选中 PRD 原文写意见，多条汇总后发送，修订后的版本和差异回到同一评审会话。你可以继续下一轮讨论，不必反复复制原文、解释位置和寻找最新文件。**需要原 Codex 任务持续参与；结束会话不代表确认文档。**

### 页面与 Spec 关联：需求随操作呈现

![页面与 Spec 对照：在实际操作旁阅读前提、结果与异常说明](assets/kada-feature-spec.png)

把页面区域或操作与 Spec 关联，在同一个工作台里操作页面、阅读说明。自动候选帮助寻找对应位置，关联由人确认；已有的规则、状态和权限通过关系展示，不由按钮外观推断。

### 一体化交付：页面、需求与关联随包交接

![只读交付：将 HTML、PRD、Spec 和页面关联一起交付查看](assets/kada-feature-delivery.png)

导出固定版本的只读 ZIP，或提供局域网只读链接，让接收者沿页面回看需求。**仅查看评审包不需要安装创作插件**，按包内说明启动查看器即可；真实业务后端与外部接口仍需另外提供。

*以上为依据工作台测试截图重新制作的功能介绍示意图，用于说明操作与价值，不是原始截图。图片随文档保存，可点击查看大图；[素材与生成说明](assets/feature-images-notes.md)。*
...
![咔哒功能流程图：产品定义、页面实现、评审交付与修改回路](assets/kada-workflow.png)
...
公开发布后，将本仓库的 [INSTALL.md](../INSTALL.md) 链接交给 Codex，并说：
...
- 公开仓库地址和 Release 下载入口尚未补齐。对外发布后，在此处补充真实链接、支持环境与操作截图。

安装步骤见 [INSTALL.md](../INSTALL.md)，维护者的打包和发布流程见 [RELEASE.md](../RELEASE.md)，更完整的现有能力说明见 [README.md](../README.md)。

---

## 作者待调整项（发布时删除）

1. 前言中“产品经理更重要”的表达力度，是否加入自己的真实经历。
2. 是否以一个具体项目贯穿介绍，展示同一操作的 PRD、Spec、页面和修改前后结果。
3. 功能地图是否保留完整链路，或只突出“产品定义、页面评审、研发交接”三部分。
4. 已加入三张品牌风格的功能介绍示意图；公开发布前可补充从真实批注到新版返回的连续录屏。
5. 发布前补真实仓库与 Release 链接，复核平台支持；如果移到仓库根目录，调整本文的相对链接。
