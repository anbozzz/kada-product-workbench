...
8. 新版发布后，同一阅读器提供新版全文、上轮修订差异及修订说明；差异的展示规则见下方“查看上轮修订差异”。已发送的等待批次继续按发送顺序交给原任务，无需因前一批发布而逐批重新发送。原任务保留每批原始引文和意见，对照当前新版处理；已被前批满足的意见说明无需再改，无法判定的冲突先向用户核对，不按旧行号覆盖新版。未发送草稿仍需核对后适配新版，冲突引用需重新选择。
...
用户在原阅读器点击“差异”，对比当前版与紧邻的上一版。默认按实际变化分块，每块显示所属章节和两版行号，保留前后各两行上下文；相邻上下文重叠或相接时合并。红色与“−”表示删除，绿色与“+”表示新增；同一段落内仅改几个词时突出这些词，未改文字保持普通样式。其余未改内容显示折叠行数，可就地展开、收起；点击“正文”返回新版全文。

比较覆盖两版的全部实际变化，包括批注选区外的修改，不按批注范围过滤。没有正文变化时提示查看修订说明。变化过大、暂时无法精确分块时明确提示，并提供可展开的两版原文供核对，不把整篇标为已修改。
...
| v0.18 | 2026-09-22 | 上轮 PRD 差异按实际变化分块，保留上下文、折叠未改正文并突出段内改词；比较包含批注范围外修改 | 用户授权本批修订，整份文档仍为草稿 |
