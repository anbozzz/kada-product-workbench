...
prd_version: v0.18
prd_revision: 1d1a1ac73a7d9d2d416c26d72df44137ed91a55900e697d9a9c86b9a4378c90f
source_refs:
  - ui/src/App.tsx
...
当前基线为 PRD v0.18；本轮多项目局域网发布（LAN-PUBLISH）已获用户实现授权并完成本机实现与回归。本文仍为草稿，不代表整份文档确认；跨设备与其他操作系统实机验收待完成。既有 DOCS-SKILL-OWNER-001 等内容保留；历史功能不因版本更新而视为已按新版全量复核。
...
关联 PRD：`需求-产品文档-相关PRD阅读器` 的“查看上轮修订差异”；来源：2026-09-22 用户授权局部展示修订。沿原“正文／差异”切换，在 `PrdRevisionDiff` 比较服务已有的 `previousSource` 与当前 `source`，不按批注锚点裁剪输入，不改变版本快照、队列、发布或正式确认协议。

- `ui/src/lib/prd-revision-diff.ts` 使用 `diff` 的逐行比较保留新增、删除和未改行。每处变化带前后各两行上下文，重叠或相接的区段合并。区段标题取首处变化所在版本最近的 Markdown ATX 标题；代码围栏内的标题样式不作为章节，无标题时显示“文档开头”。两版行号分别递增，纯新增／删除的一侧留空。
- 未改内容默认折叠为行数提示，可在原位展开、收起；上下文保持普通样式。同一连续替换区段的旧新行按顺序配对，逐字符比较，仅改动字符红／绿高亮；纯增删行完整标记，所有行同时以 −／+ 区分，兼容中文与 Unicode 字符。原“正文”返回新版全文，修订说明、批注、草稿、队列、停止及确认仍沿既有入口。
- 比较在输入变化时重新计算。逐行计算限 200 ms、编辑距离 4000；超过预算时明确“暂未完成精确分块”，提供可展开的两版原文，不输出误导性整篇红绿结果。段内比较单对长度不超过 4000 字符、编辑距离 1000、10 ms，总字符预算 50000；超过段内预算只保留准确的行级增删。统一 CRLF／LF 后没有变化时提示“本批未修改正文，请查看修订说明”。
- 验证范围：首尾与中间多处修改、批注外变化、相邻变化合并、上下文展开收起、中文改词、纯增删／空文档／重复行／尾换行、大文档稀疏修改、预算耗尽、窄抽屉，以及原工作台的批注发送、队列、正文返回、刷新与确认。

本批验证（2026-09-22）：`npm run verify` 通过，含 208 项 Node 测试、19 条真实浏览器旅程和差异检查。原评审入口验证了三处零散变化、批注范围外修改、段内高亮、折叠展开、窄屏及返回正文；PRD 与本节通过独立局部复核。此次交付为仓库源码与构建产物，不代表本机已安装插件或既有运行会话已更新。
